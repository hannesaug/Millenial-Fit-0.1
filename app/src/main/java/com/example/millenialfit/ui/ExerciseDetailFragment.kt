...
import com.example.millenialfit.databinding.FragmentExerciseDetailBinding
...
class ExerciseDetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.exerciseList.observe(viewLifecycleOwner){
            for (i in it){
                if (i.id == getID){
                    binding.exerciseImageview.load(i.image)
                    binding.exerciseTitleTextview.text = i.title
                    binding.exercisePlanTextView.text = i.workoutPlan
                }
            }
        }
